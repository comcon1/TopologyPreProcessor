...
//
//  long tail algorithms implementations
//
...
ublas::vector<int> generate_long_tail1(OBMol &mol, bool verbose) {
...
AtomArray mol_renum(OBMol &mol, AtomArray &ar, ublas::vector<int> tail, bool hex_flag) {
  ...
  for (int p = 1; p < tail.size(); ++p) { // cycle over longtail head atoms
    ...
    tat.coord(1) = pQ->GetY();
    tat.coord(2) = pQ->GetZ();
    tat.atom_name = lexical_cast<string>(k)+"H"+mc_numerer(h, hex_flag);
    BOOST_CHECK(A.insert(tat).second);
  }
    int stop = 0, m = 1, sh = 0;
    schain.resize(2);
    ...
    if (m != 1) {
      ...
      for (int q = 1; q < schain.size(); q++) {
        ...
        tat.atom_name = an2str(mol.GetAtom(schain(q))->GetAtomicNum(),
            tat.atom_name) + mc_numerer(h, hex_flag);
        BOOST_CHECK(A.insert(tat).second);
        k = 0;
    ...
    // ----------------------------------------
  {
    ...
    os << "Zoidberg renumeration algorithm had finished its work.\n";
    runtime.log_write(os.str());
  }
  return A;
} // end pdb_renum)
...
void __mol_renum1(OBMol &_mol, AtomArray &_ar, ublas::vector<int> &_tail,
  ...
  while (true) {
    ublas::vector<int> newtail = generate_long_tail1(_mol, _tailed, _tail(p), hex_flag);
    if (newtail.size() == 1) break;
    __mol_renum1(_mol, _ar, newtail, _tailed, _A, _n, _h, hex_flag);
  }
} // end cycle over longtail

} // end of recursion function

// main function for comcon 1 algorithm
AtomArray mol_renum1(OBMol &mol, AtomArray &ar, ublas::vector<int> tail, bool hex_flag) {
  ...
  // STARTING RECURSION
  __mol_renum1(mol, ar, tail, tailed, A, n, h, hex_flag);
